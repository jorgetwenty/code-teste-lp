...
read from it. Wired for **GringX** (2026-07-08): name `GringX`, tagline
"Turn data into decisions", the hero description as the site description,
`@gringx`. The home route (`app/page.tsx`) exports
`generateMetadata({ title: \`${name} — ${tagline}\`, url: "/" })`;
`public/manifest.json` and `browserconfig.xml` carry the matching PWA
name/colours (black theme).
