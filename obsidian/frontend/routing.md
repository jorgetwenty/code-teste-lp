...
- `src/app/layout.tsx` — the **root layout**. Holds the provider tree
  (`ScrollLayout` → `ReducedMotion` / `Cookie` → children), loads the local
  Mulish font and `globals.css`, exports `metadata` + `viewport`, and renders the
  JSON-LD script. (Proportional scaling is CSS-driven now — `<AdaptiveGrid>` is
  no longer mounted; ADR-0016.) See [[data-flow]].
