...
> [!note] WebGL is a separate layer
> The spring-only rule governs **DOM/CSS** motion. **GPU shader animation**
> (`three` — driven by a `requestAnimationFrame` uniform clock) is a distinct
> rendering layer and is **not** covered by it — springs remain the only way to
> animate the DOM. See [[components/three]] and [[decisions-log]] ADR-0014.
