...
## ADR-0016 — Proportional scaling: single 1440 base, CSS-driven (amends ADR-0008)

- **Status:** Accepted
- **Date:** 2026-07-07

**Context.** ADR-0008's grid used a per-range base (top range anchored to **1920**)
and drove scale-**up** above the largest breakpoint with the JS `<AdaptiveGrid>`
(damped, `coef` 0.6666). But the real UI (the GringX hero) is authored at **1440**,
so on a >1440 monitor it rendered small/left instead of filling the viewport, and
the damped JS up-scale never made it fully proportional. The desired behaviour is
simply: every element occupies the same share of the viewport as in the 1440
Figma, at any width.

**Decision.**
- **One 1440 base.** `grid.config.ts` drops the 1920 breakpoint; `1440` is the top
  base (`GRID_BASE_WIDTH` → 1440). Tablet/mobile bases (1024, 360) stay.
- **CSS-driven, uncapped.** `globals.css` sets `html { font-size: 1.111111vw }`
  (= 16 × 100 / 1440) with no upper cap, so 1rem = 16px at 1440 and scales **up**
  above it and down to 1024 — pure CSS, flash-free and SSR-correct (no hydration
  jump, no resize listener).
- **`<AdaptiveGrid>` unmounted.** Its JS scale-up is now redundant, and its
  default damping would *cap* the proportional scale-up. Removed from the root
  layout; the component + `useAdaptiveGrid` hook are **retained** as an opt-in for
  the rare case you want *damped* scale-up on ultrawide (`coef` < 1) — mount it
  then. This supersedes ADR-0008's "scale-up = JS" half; scale-down was already
  CSS.

**Consequences.** The layout is proportional to the 1440 design everywhere ≥1024,
filling large monitors. Tablet/mobile (<1024) keep their own bases, so the single
1440 hero will overflow there until a responsive layout is built — unchanged from
before. ADR-0008's breakpoint-mirroring rule (config ↔ `globals.css`) still holds.

---

## ADR-0015 — GringX hero: single dark theme + DOM overlay on the WebGL scene

- **Status:** Accepted
- **Date:** 2026-07-07

**Context.** The first real UI is the GringX marketing hero (Figma node 551:451),
whose background *is* the WebGL nebula ([[components/three]]). The starter shipped
a light/dark-adaptive minimal theme; the design is a single dark screen.

**Decision.**
- **Single dark theme.** `globals.css` `:root` is now the dark theme (`#000` /
  `#fff`); dropped the `prefers-color-scheme` split. Added tokens `--muted`,
  `--border`, `--accent`, `--text-hero`, `--text-stat`, `--spacing-edge`. The body
  no longer force-centres content (that was placeholder for the bare nebula).
- **Hero is a DOM overlay.** `HomeView` layers `NebulaScene` (`fixed inset-0`)
  under `SiteHeader` + `Hero`; the overlay paints on top by DOM order (+ `z-10`),
  and the nebula keeps its mouse interaction via its `window` listener. Content
  is data-driven (`src/data/mocks/home.ts`); a reusable `Button` landed in
  `components/ui/`.
- **First-party SVGs via `next/image`.** Enabled `dangerouslyAllowSVG` with a
  locked `contentSecurityPolicy` in `next.config.ts` to serve the provided
  `logo.svg` / `icon.svg`. Safe because the SVGs are our own, not user input.

**Consequences.** The app is dark-only now (revisit with an ADR if theming is
needed). `dangerouslyAllowSVG` is acceptable strictly for first-party assets —
never point `next/image` at untrusted SVGs. **Fidelity caveat:** exact
colours/type could not be pulled from Figma (Dev Mode "allowed directories"
blocked `get_design_context`), and the design's font files were absent — values
are estimated and the typeface is unconfirmed (using Onest). Flagged in
[[changelog]] and [[components/hero]] for a design-review pass.

---

## ADR-0014 — Adopt `three` for WebGL 3D; nebula sphere on the home view

- **Status:** Accepted
- **Date:** 2026-07-07

**Context.** The first real page is a 3D scene: a shader-driven nebula on a
sphere, black void background, cursor-interactive. Nothing in the stack does
GPU/WebGL — `@react-spring/web` animates DOM/CSS, `spring-text-engine` animates
text. The source material was a self-contained Three.js **full-screen** nebula
(a `PlaneBufferGeometry` fragment shader in screen space), but the requirement is
the nebula **on a 3D sphere**, so a verbatim port was never the goal.

**Decision.**
- **Add `three` (`^0.185`) + `@types/three`** as the WebGL layer. Hard rule #1
  ("all motion is `@react-spring/web`, no CSS transitions/keyframes, no
  `framer-motion`") governs **DOM/CSS** motion. GPU shader animation driven by a
  `requestAnimationFrame` uniform clock is a **separate rendering layer** and is
  explicitly **not** in scope of that rule. Spring/text motion stays the only way
  to animate DOM. Noted in [[tech-stack]] and [[animation-system]].
- **Raw `three` in a `"use client"` leaf**, not `@react-three/fiber`. The scene
  is one mesh + a bloom compositor with a hand-written render loop; R3F's
  reconciler adds three packages and fights the imperative loop for no gain. The
  component owns full lifecycle teardown (rAF cancel, geometry/material/renderer
  dispose, listener removal). Lives in a new `src/components/three/` domain
  folder; catalog note [[components/three]].
- **Shader adapted to the sphere surface.** The one structural change from the
  source: the 2D nebula coordinate is a **stereographic projection of the
  object-space normal** instead of `gl_FragCoord`. This paints the nebula, stars
  and hero sparkle onto the mesh with no UV seam and no pole pinch, and they flow
  across the surface as it spins. A fresnel term adds limb darkening + an
  atmosphere rim so it reads as a lit orb. Cursor interaction raycasts onto the
  sphere and stirs/glows at the hovered point.
- **Compositor simplified** from the source's three-composer selective-bloom
  rig (which existed to bloom a subset of a full-screen scene) to a single
  `EffectComposer`: `RenderPass` → `UnrealBloomPass` → `OutputPass`. One glowing
  sphere on black needs nothing more.
- **Config as props.** The spec's fixed `CONFIG` (colours + tunables) is a typed
  `NebulaConfig` with the spec values as defaults, overridable per-instance —
  satisfies "content via props" (these are WebGL values, not `globals.css`
  design tokens, so the token rule does not apply).

**Consequences.** First WebGL dependency in the tree; first 3D scene. `yarn lint`,
`tsc`, and `yarn build` are clean (`/` prerenders static). The home view is no
longer empty. If a future scene needs declarative composition or physics, R3F can
be revisited with a new ADR. See [[components/three]] and [[changelog]].

**Extension (same day).** The scene gained a DRACO-compressed **hands** model
cradling the orb (`GLTFLoader` + `DRACOLoader`, decoder vendored to
`public/draco/`, served same-origin — no new npm dependency) and a **reflection**
of the orb on the hands via a per-frame `CubeCamera` env map + IBL. `public/**`
was added to the ESLint ignore list (the decoder is third-party minified JS).
